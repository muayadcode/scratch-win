...
export const checkLastPlayed = async (email) => {
    ...
    if (local && local.lastPlayed != null) {
...
export const saveLastPlayed = async () => {
    ...
    // Update localStorage immediately so 72hr check works offline
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            await updateDoc(doc.ref, { lastPlayed });
        }
    } catch (error) {
...
const addPrizeToUser = async (prize, email) => {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            await updateDoc(doc.ref, { prizesWon });
        }
    } catch (error) {
