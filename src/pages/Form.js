...
class FormData {
  ...
  setMinorFields(guardianNameFirst, guardianNameLast, guardianEmail, guardianNumber) {
    this.isMinor = true;
...
class FormField {
  ...
  check(value) {
    // console.log(`Checking ${this.name} with ${value}`)
...
function Form() {
  ...
  const [errors, setErrors] = useState({ fName: "", lName: "", email: "", phone: "", password: "", st: "", postal: "", province: "", city: "", date: "", guardianfName: "", guardianlName: "", guardianEmail: "", guardianPhone: "" });
  ...
  let formFields = [
    ...
    new FormField("Email", "Please provide a valid email address.", emailReg, "email"),
    new FormField("Date", "*Invalid Date", hasDate, "date"),
    new FormField("Password", "Must contain at least one number, upper and lower case letter, and at least 6 characters.", passwordCheck, "password"),
  ];

  let formFieldsIsMinor = [
    new FormField("Guardian's First Name", "*Invalid Name", charReg, "guardianfName"),
    new FormField("Guardian's Last Name", "*Invalid Name", charReg, "guardianlName"),
    new FormField("Guardian's Email", "*Invalid Email", emailReg, "guardianEmail"),
    new FormField("Guardian's Phone", "*Invalid Phone", phoneReg, "guardianPhone"),
  ]

  let formBoxes = [
    new FormField("Rules", "*Please consent to the rules and regulations.", (data) => data, "rules"),
    new FormField("Sponsors", "sponsors")
  ...
  const formCheck = (e) => {
    ...
    let errorsFound = 0;
    let errorsTemp = { fName: "", lName: "", email: "", phone: "", password: "", st: "", postal: "", province: "", city: "", date: "", guardianfName: "", guardianlName: "", guardianEmail: "", guardianPhone: "" };
    ...
    let formFieldsTemp = {
      ...
      rules: e.target.elements["rules"].checked
    }
    ...
    if (errorsFound > 0) {
      ...
      if (isMinor) {
        navigate('/success', { state: { formData: saveFormData(formFieldsTemp, formFieldsIsMinorTemp) } });
      } else {
        navigate('/success', { state: { formData: saveFormData(formFieldsTemp) } });
      }
      console.log("PASS");
    }
  };
      ...
      <form id="myForm" onSubmit={formCheck} >
        <fieldset className='formStyle'>
          <div className="child50">
            ...
            <p id="fNameError" className="error">{errors.fName}</p>
          </div>
          <div className="child50">
            <label htmlFor="lName">Last Name: </label>
            <input type="text" id="lName" placeholder="Last Name" />
            <p id="lNameError" className="error">{errors.lName}</p>
          </div>
          <div className="child100">
            <label htmlFor="st">Address Line: </label>
            <input type="text" id="st" placeholder="Address Line" />
            <p id="stError" className="error">{errors.st}</p>
          </div>
          <div className="child30">
            <label htmlFor="postal">Postal: </label>
            <input type="text" id="postal" placeholder="Postal Code" />
            <p id="postalError" className="error">{errors.postal}</p>
          </div>
          <div className="child30">
            <label htmlFor="province">Province: </label>
            <input type="text" id="province" placeholder="Province" />
            <p id="provinceError" className="error">{errors.province}</p>
          </div>
          <div className="child30">
          ...
          <div className="child100">
            ...
            <p id="emailError" className="error">{errors.email}</p>
          </div>
          <div className="child100">
            <label htmlFor="password">Password: </label>
            <input type="text" id="password" />
            <p id="passwordError" className="error">{errors.password}</p>
          </div>
          ...
          <section className='checkBoxes'>
            <div className=" checkBox">
              ...
              <label id="rulesLabel" className="disabled" htmlFor="rules">I Consent to Rules and Regulations listed above.</label>
            </div>
        ...
        <button id="formSubmit" className='blueButton'>Submit</button>
      </form>
